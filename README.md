...
受信と送信はどの部屋でも同じですが、**訳が自分にどう見えるか／中継するかどうかは、自分が部屋を
立てた（ホスト）か、他の人の部屋に参加した（クライアント）かで変わります**
（[ホストとクライアントで変わること](#ホストとクライアントで変わること)）。
...
| `/` `!` `.` で始まる発言 | 翻訳しない（コマンド用） |
| **自分がホストのとき** | 同僚ドワーフの発言の訳を全員のチャットにも流す（[下記](#ホストとクライアントで変わること)） |
...
同じ設定のままでも、**自分が部屋を立てた（ホスト）ときと、他の人の部屋に参加した（クライアント）とき**で挙動が変わります。
理由は1つです。訳をチャット欄に出すのに使っているゲームの機能は、**クライアントから呼ぶと自分だけに、
ホストから呼ぶと全員に**表示されます。

| | ホスト（マルチ） | ホスト（ソロ） | クライアント |
|---|:---:|:---:|:---:|
| 自分の発言を翻訳して送る | 〇 | 〇 | 〇 |
| 受信の訳が自分のチャット欄に出る | 〇 ※1 | 〇 | 〇 |
| 訳を全員のチャットに流す（中継） | 〇 | × ※2 | × ※3 |

※1 出ますが、**中継の行として出ます**。受信用の書式（`DRGT_INCOMING_FORMAT`）ではなく、
全言語を1行にまとめた形で、同じ行が全員にも見えます
（→ [自分がホストのときの中継](#自分がホストのときの中継)）。
中継を切っている場合、または中継の言語（`DRGT_RELAY_TARGETS`）から日本語を外した場合は
出ないので、`settings.ini` の `DRGT_OVERLAY_ENABLED=true` で小窓に出せます。
※2 読む相手がいないためです。訳は日本語だけ作って自分の画面に出します。
※3 他人の部屋のチャットを勝手に埋めないためです。

**自分の発言の翻訳（送信）は、どの立場でも同じです。** 立場で変わるのは、
受信の訳が**どういう形で**出るか（※1）と、その訳を全員に流すかどうかの2つです。
...
> **`ja` は残してください。** ホスト（マルチ）では、自分が読む訳もこの行から読むためです
> （[※1](#ホストとクライアントで変わること)）。`ja` を外すと自分の画面にも訳が出なくなります。
...
: 仕様です。ホストのチャット欄に出したものは全員に届くので、受信の訳は中継の行として
  1行にまとめて流しています（[ホストとクライアントで変わること](#ホストとクライアントで変わること)）。
