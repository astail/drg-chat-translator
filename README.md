...
[冒頭の表](#drgtranslate)を立場ごとに詳しく書いたものです。挙動が変わる理由は1つで、
訳をチャット欄に出すのに使っているゲームの機能が、**クライアントから呼ぶと自分だけに、
ホストから呼ぶと全員に**表示されるためです。

**ホスト（マルチ）** — 受信の訳は、受信用の書式（`DRGT_INCOMING_FORMAT`）ではなく
**中継の行として**出ます。全言語を1行にまとめた形で、同じ行が全員にも見えます
（→ [自分がホストのときの中継](#自分がホストのときの中継)）。
中継を切っている場合、または中継の言語（`DRGT_RELAY_TARGETS`）から日本語を外した場合は
何も出ないので、`settings.ini` の `DRGT_OVERLAY_ENABLED=true` で小窓に出せます。

**ホスト（ソロ）** — 受信の訳はそのままチャット欄に出ます（全員に配信されますが、
その「全員」が自分だけなので）。中継はしません。読む相手がいないのに 4言語ぶん訳しても
無駄なので、訳は日本語だけ作ります。

**クライアント** — 受信の訳はチャット欄に出ます。中継はしません。
他のドワーフが立てた部屋のチャットを勝手に埋めることはありません。
