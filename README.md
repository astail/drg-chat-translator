...
| | ホスト（マルチ）<br>自分の部屋に他の人がいる | ホスト（ソロ）<br>自分ひとり | クライアント<br>他の人の部屋に参加 |
|---|:---:|:---:|:---:|
| 自分の発言を翻訳して送る | 〇 | 〇 | 〇 |
| 受信の訳が自分のチャット欄に出る | × ※1 | 〇 | 〇 |
| 訳を全員のチャットに流す（中継） | 〇 | × ※2 | × ※3 |

※1 ホストのチャット欄に出すと全員に届くので、受信の訳は中継として1行にまとめて流します。
その中に日本語も含まれるので、それを読んでください
（→ [自分がホストのときの中継](#自分がホストのときの中継)）。
中継を切っている場合は `settings.ini` の `DRGT_OVERLAY_ENABLED=true` で小窓に出せます。
※2 読む相手がいないためです。訳は日本語だけ作って自分の画面に出します。
※3 他人の部屋のチャットを勝手に埋めないためです。

**自分の発言の翻訳（送信）は、どの立場でも同じです。** 立場で変わるのは下の2行、
「受信した訳が自分にどう見えるか」と「中継するかどうか」だけです。
...
: 仕様です。ホストのチャット欄に出したものは全員に届くので、受信の訳は中継として
  1行にまとめて流しています（[ホストとクライアントで変わること](#ホストとクライアントで変わること)）。
  中継が有効なら、その行に日本語が含まれるので読めます。
