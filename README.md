...
合言葉のような文字列で、初めて `DRGTranslate.exe` を起動したときに出る質問の中で貼り付けます。
先に用意しておかないと、その途中で止まってしまいます。
...
答えた内容は exe と同じフォルダの `settings.ini` に保存されます。2回目以降は `settings.ini` が
あるので、この質問は出ずにすぐ常駐状態になります。

**言語・ゲームの場所・APIキーを選び直したいときや、MOD を入れ直したいとき**は、
exe と同じフォルダにある `settings.ini` を削除してから `DRGTranslate.exe` を起動してください。
上の `[1]`〜`[6]` の質問がもう一度出ます
...
1. ゲームを終了します。
2. 新しい版の zip をダウンロードして展開します。
3. 前の版のフォルダにある `settings.ini` を、新しい版のフォルダにコピーします。
4. 新しい版の `DRGTranslate.exe` を起動します。

起動したときに、ゲームフォルダの MOD が古ければ、自動で新しい版に入れ替わります
（黒い窓に「ゲームフォルダの MOD を 0.8.1 から 0.8.2 に更新しました」のように出ます）。
前の MOD は `Mods\DRGTranslate.bak` に残ります。ゲームを起動したまま入れ替えた場合は、
ゲームを終了してもう一度起動すると新しい MOD が読み込まれます。

ゲームの場所を自分で入力した場合など、ゲームフォルダが見つからないと自動では入れ替わりません。
そのときは黒い窓の案内のとおり、ゲームを終了し、`settings.ini` を削除してから
`DRGTranslate.exe` を起動し直してください。
...
（初めて `DRGTranslate.exe` を起動したときに作られます）。書き換えて保存したら、`DRGTranslate.exe` を起動し直してください。
...
| キー | 説明 |
|---|---|
| `DRGT_UI_LANG` | 初めて起動したときの質問と、黒い窓のメッセージの言語。`ja` / `en` / `ko` / `zh` / `zh-tw` / `ru`。未設定なら `DRGT_INCOMING_TARGET` と同じ言語 |
...
**ふつうは初めて起動したときの質問 `[1]` で選ぶだけで済みます。** 選んだ言語に合わせて
...
`settings.ini` を直してください（`settings.ini` を削除して `DRGTranslate.exe` を起動し直すと、[1] から選び直せます）。

> 0.7.0 までの版は、初めて起動したときに `DRGT_INCOMING_SKIP_LANGUAGES` と `DRGT_RELAY_MAX_LANGS` も
> 書いていました。この2行が `#` 無しで残っていると、言語を直しても追従しません。
> 行頭に `#` を付けるか、`settings.ini` を削除して `DRGTranslate.exe` を起動し直してください。
> ただし `[1]` で繁体字中国語を選んだ場合の `DRGT_INCOMING_SKIP_LANGUAGES=zh` は
...
`settings.ini` を削除して `DRGTranslate.exe` を起動し直したときや、[新しい版にしたとき](#新しい版にするとき)は
MOD が入れ直され、編集していた `config.lua` は `Mods\DRGTranslate.bak` に退避されます。
...
- 初めて起動したときの質問と黒い窓のメッセージは、配布と同じ手順でビルドした exe を Windows で動かし、
  6言語すべてで確認しています。
...
: まず MOD を切り分けます。`FSD\Binaries\Win64\dwmapi.dll` の名前を
  ...
  どちらも v0.2.1 以降は、初めて起動したときに自動でやります。古い版で入れた場合は、
  `settings.ini` を削除してから `DRGTranslate.exe` を起動し、最初と同じ質問に
  答え直すと直ります。
...
| | ライセンス | 扱い |
|---|---|---|
| [UE4SS](https://github.com/UE4SS-RE/RE-UE4SS) | MIT | **同梱していません**。初めて起動したときに公式リリースから取得します |
