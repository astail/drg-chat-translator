...
| サービス | 料金 | こんな人に | 登録・APIキーの発行 |
|---|---|---|---|
| **DeepL** | 無料のお試し枠あり。使い切ったら有料プラン | まずは無料で試したい | https://www.deepl.com/pro-api |
| **Claude**（Anthropic） | 使った分だけ支払う従量課金。既定の Claude Haiku 4.5 なら、**1000回翻訳して約 $0.5** | スラング・略語・誤字まで訳してほしい | https://platform.claude.com/settings/keys |
| **OpenAI** | 使った分だけ支払う従量課金。既定の gpt-4o-mini なら、**1000回翻訳して約 $0.1 未満** | スラング・略語・誤字まで訳してほしい | https://platform.openai.com/api-keys |
...
どれを選べばいいか迷ったら、[翻訳サービスの選択](#翻訳サービスの選択)も参考にしてください。
...
3つから選びます。どれも APIキーが必要です（料金は[始める前に](#始める前に翻訳サービスの-apiキーを用意する)を参照）。

| provider | 特徴 |
|---|---|
| `deepl`（既定） | 機械翻訳としては最も自然。**無料のお試し枠あり** |
| `claude` | **スラング・略語・誤字に強い**。`gg` `bulk inc` `res me` を文脈で訳せる |
| `openai` | **スラング・略語・誤字に強い**。`gg` `bulk inc` `res me` を文脈で訳せる。OpenAI互換エンドポイント（ローカルLLM等）にも向けられる |
