...
| **F9** | 翻訳のON/OFF切り替え（OFF にすると中継の順番待ちも捨てます）<br>ホストのときはゲーム内に出せないので、黒い窓に `翻訳 OFF` と出ます |
...
発言者の言語は除いて訳し、**1言語につき1行**流します
（1行にまとめるとチャットの文字数制限に引っかかるため）。

```
Karl: watch out, swarm incoming          ← 英語の発言
You: [JP] Karl: 気をつけろ、大群が来るぞ
You: [KR] Karl: 조심해, 무리가 온다
You: [ZH] Karl: 小心，虫群来了
```
...
| `host_relay.interval_ms` | 中継を1行ずつ送る間隔。既定 `700` |
...
- **UE4SS を模したスタブによる MOD ロジックの通し確認**（`tools/mock_test.lua`）
  ...
  （発言者の言語を除いた3言語が1行ずつ送られる／クライアントでは送られない／
  中継行をさらに翻訳しない／中継行の送信者名） — クライアント24項目・ホスト28項目すべて PASS
