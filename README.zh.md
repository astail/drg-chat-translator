...
从 [**Releases**](https://github.com/astail/drg-translation/releases) 下载
...
はじめに.txt          三步指引（日语）
README.md 等          各语言的 README / LICENSE
```
...
- 术语表（`glossary.json`）的译文是日语，所以把接收的翻译目标设为日语以外时不会被使用
  （`Rock and Stone!` 之类的口号会原样显示）。
...
| 项目 | 说明 |
|---|---|
| `outgoing.enabled` | 是否翻译并发送自己的发言 |
| `outgoing.ignore_prefixes` | 以这些字符开头的发言不翻译 |
| `outgoing.min_length` | 短于这个字数的发言不翻译 |
| `incoming.skip_own` | 不翻译自己的发言 |
| `host_relay.enabled` | 作为房主时是否把同伴发言的译文推给所有人 |
...
- 安装过程和黑色窗口的消息已在 6 种语言下逐一验证（在实际游戏中的确认尚未进行）。
- **俄语和繁体中文能否用游戏字体显示尚未确认**（日语、韩语、简体中文已确认）。
- **多人房间里转发能否送达同伴，尚未确认。**
- 小窗（`DRGT_OVERLAY_ENABLED=true`）在实机上的显示效果也未确认。

详情请见 [docs/TESTING.md](docs/TESTING.md)（日语）。
...
**译文不显示**
: 请确认 `FSD\Binaries\Win64\UE4SS.log` 中有两行 `hook 登録: ...`。
  如果没有，可能是游戏更新导致函数名变了。
...
**只想停止翻译自己的发言**
: 把 `config.lua` 的 `outgoing.enabled` 设为 `false`，只保留接收翻译。
...
| [docs/TESTING.md](docs/TESTING.md) | 验证情况的详细内容（也包括哪些尚未验证） |
