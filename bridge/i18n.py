...
_M.update({
    ...
    "p.effort_ignored": {
        "ja": "%s は effort に対応していないため無視します（4.6 以降のモデルのみ）",
        "en": "%s does not support effort, so it is ignored (models 4.6 and newer only)",
        "ko": "%s 는 effort 를 지원하지 않아 무시합니다 (4.6 이후 모델만 해당)",
        "zh": "%s 不支持 effort，将忽略该设置（仅 4.6 之后的模型支持）",
        "zh-tw": "%s 不支援 effort，將忽略該設定（僅 4.6 之後的模型支援）",
        "ru": "%s не поддерживает effort — параметр игнорируется (только модели 4.6 и новее)",
    },
    "p.unknown_provider": {
