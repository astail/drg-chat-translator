...
- **表示言語の切り替え**（2026-09-19 / `bridge/i18n.py` を入れた変更）
  - カタログの機械的な検査：111キー × 6言語（ja/en/ko/zh/zh-tw/ru）に抜けが無いこと、
...
- **多言語化したあとの実機での通し確認。** 変更したのは文言の差し替えと設定の
  ...
  まだしていません
- **Windows で OS の言語を聞く経路**（`i18n._windows_ui_lang()`。
  `GetUserDefaultUILanguage` + `LCIDToLocaleName`）。この環境では Windows の
  Python を動かせないため、返ってくる文字列を `normalize()` に通すところまでしか
  確認していません。失敗したときは `None` を返して英語になります
- **ロシア語・繁体字中国語がゲーム内フォントで描画されるか。** 日本語・韓国語・
  簡体字中国語は実機で確認済みですが、この2つは未確認です（□□□ になる場合は
  ゲームの言語設定をその言語に合わせてください）
