...
中継行は 1言語 1行で、`700ms` 間隔で1行ずつ送ります。まとめて送ると
1フレームで `Server_NewMessage` を連打することになり、チャットも一瞬で流れます。
自分が流した行は `own_sent` で、他人（別のホスト）が流した行は
`^[XX] ` という書式で判定して、翻訳のループを防いでいます。
...
|---|---|
| `HELLO <version>` | |
| `RES <id> <kind> <srclang> <text> [中継行...]` | 結果。`text` が空なら「何もしない」。6番目以降はホストが全員に流す行（1言語1行） |
| `ERR <id> <message>` | 失敗 |
